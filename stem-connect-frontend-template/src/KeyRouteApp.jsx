...
import { createSession, getArrivals, boardTrip, getProgress, stopsNear, stopsSearch } from "./keyrouteApi";

const fmt = (s)=> s==null ? "—" : (s<60 ? `in ${Math.max(5,Math.round(s/5)*5)}s` : `in ${Math.round(s/60)} min`);
...
export default function KeyRouteApp(){
  ...
  const pickNearby = async()=>{
    // NOTE: you can replace coords with navigator.geolocation later
  ...
  const start = async()=>{
    ...
    const s = await createSession(origin.stop_id, dest.stop_id, "B"); // route_id is optional
  ...
  const confirm = async(trip_id)=>{
    ...
    poll.current = setInterval(async ()=>{
      ...
      if(p.state === "ARRIVED"){ clearInterval(poll.current); }
    }, 3000);
  };
  ...
  return (
    <div className="max-w-md mx-auto p-4 space-y-4">
      ...
      <div className="space-y-1 p-3 border rounded-2xl">
        ...
        <button className="px-3 py-2 border rounded-xl" onClick={pickNearby}>Use Nearby</button>
      ...
      <div className="space-y-1 p-3 border rounded-2xl">
        ...
        <form onSubmit={(e)=>{ e.preventDefault(); const q=e.target.q.value; searchDest(q); }}>
          <input name="q" className="px-3 py-2 border rounded-xl w-full" placeholder="Search stop (e.g., Park Street)" />
        </form>
      ...
      {state === "AWAITING_BOARD" && (
        <div className="space-y-2 p-3 border rounded-2xl">
          ...
          {arrivals.map(a=>(
            <div key={a.trip_id} className="flex items-center justify-between p-2 border rounded-xl">
              <div>
                <div className="font-medium">{a.headsign}</div>
                <div className="text-sm opacity-70">To origin: {fmt(a.eta_sec)}</div>
              </div>
              <button className="px-3 py-2 rounded-xl bg-black text-white" onClick={()=>confirm(a.trip_id)}>
                Confirm Boarding
              </button>
            </div>
          ))}
      ...
      {sessionId && (
        <div className="space-y-2 p-3 border rounded-2xl sticky bottom-4 bg-white/90 backdrop-blur">
          ...
          <div className="text-lg font-semibold">{state.replaceAll("_"," ")}</div>
